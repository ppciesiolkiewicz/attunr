...
A guided, progressive path through vocal technique. Replaces the free-form trainer with a structured sequence of exercises — first mastering individual tones, then combining them into flowing sequences.
...
2. **Page** parses step id from URL, validates (1–116), finds the exercise in `JOURNEY_EXERCISES`
3. **Render** by exercise type:
   - **`pitch-detection`** / **`pitch-detection-slide`**: PitchCanvas + exercise UI (detection, progress, Hear tone, Next/Skip)
   - **`breathwork-farinelli`**: Breathing cycle UI (no pitch detection)
...
Journey is a separate view from the Train tab, accessible from the bottom navigation. From the Journey list view, tapping an exercise navigates to `/journey/[id]`. From a step, "← Journey" returns to the list; Previous/Next move between steps via URL.

---

## Exercise types

| exerciseTypeId | Rendering | Purpose |
|-------------|-----------|---------|
...
| `breathwork-farinelli` | Breathing cycle UI | Farinelli breathing cycles — no pitch detection |

**Intro exercises** have no canvas. **Active exercises** (`pitch-detection`, `pitch-detection-slide`, `breathwork-farinelli`) show the relevant UI and success criteria. See [Exercise Config Flow](./exercise-config-flow.md) for detailed data flow, canvas selection, and progress models.

---

## Part structure (20 parts, 116 exercises — Parts 11–12 temporarily disabled)
...
Most parts start with a `learn` exercise followed by active exercises. Some later parts skip the learn exercise when no new concept is introduced.

> **Parts 11–12 (Chakras)** are temporarily disabled. Exercise IDs 56–67 are reserved but not in the active `JOURNEY_EXERCISES` array. Navigation uses `getNextExerciseId()` to skip the gap (Part X exercise 55 → Part XIII exercise 68).

---

## Part complete modal

When the user completes the **last step of a part** (e.g. exercise 7 for Part II):
...
On desktop, the part name is shown alongside the exercise title in the header.
...
| Type | Criteria |
|------|----------|
| `pitch-detection` (single note) | Pitch within ±3% of target for cumulative hold time (configurable per exercise via `notes[0].time`). Progress arc fills. |
...
| Location | Purpose |
|----------|---------|
| `src/constants/journey/` | Exercise data (one file per part) |
| `src/constants/journey/types.ts` | `JourneyExercise`, `ExerciseTypeId`, `Note`, `TechniqueId`, etc. |
| `src/constants/journey/index.ts` | Re-exports `JOURNEY_EXERCISES`, `JOURNEY_CONFIG`, etc. |
...
Journey progress (highest completed exercise) is stored in localStorage. Key: `attunr.journeyStage` — integer 1–116. Used to show "Completed" on past steps and to track the current frontier.
